...
from NLP import preprocessing_web as pre
from NLP import data_util as du
...
def validate_for_generator(problem_data: pd):
    ...
    if total_available_hours < study_hours:
        c.send_output(
            "It looks like the hours needed to study exceed the hours in the plan.\n Please enter a bigger duration of the study plan.")
        return False
    elif (total_available_hours-constraint_hours) < study_hours:
        c.send_output(
            "It looks like the given restrictions make the needed hours to study exceed the hours in the plan.\n")
        c.send_output("Should we create the plan without the restrictions?")
        ...
        if confirmation == True:
            ...
            problem_data.data["soft_constraints"]["no_study_hours"] = []
            c.send_output(
                "The Restrictions have been erased, try to add them again")
            return False
...
def handle_context_main(new_context, ProblemData: pd, sentence: str):
    ...
    if "-total_time" in new_context:
        ...
        c.send_output("When do you want to start?(For Example: July 10th)")
        while True:
            date_input = c.await_for_message()
            date = pre.tag_date(date_input)
            if type(date[0]) != list:
                c.send_output("We are going to start from: "+date[0])
        ...
        c.send_output("cool, we are going to make your hours with a duration of" +
        ...
        while hours_per_day > 8:
            c.send_output(
                "Studying for more than 8 hours per day is not healthy, try with less hours please")
            hours_per_day = abs(pre.number_from_text(c.await_for_message()))
        c.send_output("We are going to plan your days with " +
...
def handle_context_no_study_hours(sentence: str, ProblemData: pd, constraint_type):
    ...
    c.send_output(
        "Great!, well take in consideration not to assign you study time during these hours:\n"+time_str)

    c.send_output(
        "Can you tell me for which date or dates do you want this to be taken in consideration?")
    Everyday = False
    date_input = pre.tag_date(c.await_for_message())
    dates_str_list = []
...
def keep_editing(ProblemData: pd, subject: Subject):
    ...
    c.send_output(keep_editing_str)
...
def ask_for_confirmation():
    confirmation = c.await_for_message()
