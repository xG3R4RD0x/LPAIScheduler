...
def create_test_subject(problem_data: ProblemData, subject_name: str):
    subject = Subject(subject_name)
    du.add_subject(problem_data, subject)


def create_test_nsh(problem_data: ProblemData):
    pd = problem_data
...
def create_test_nsd(problem_data: ProblemData,):
...
def create_test_problem_data():
    ...
    # adding subjects
    create_test_subject(pd, "Math")
    subject = du.get_subject_by_name(pd, "Math")
    info = {"number_of_units": 10, "hours_per_unit": 5}
    du.update_subject(subject, info)
    create_test_subject(pd, "Literature")
    subject = du.get_subject_by_name(pd, "Literature")
    info = {"number_of_units": 6, "hours_per_unit": 6}
    du.update_subject(subject, info)
    create_test_subject(pd, "Chemistry")
    subject = du.get_subject_by_name(pd, "Chemistry")
    info = {"number_of_units": 8, "hours_per_unit": 5}
    du.update_subject(subject, info)
    # total_time
